...
void sha_string(SHA_INFO *sha_info, const char *strin, int strlen)
{
    const char *p = strin, 
		 *e = strin + strlen;

    sha_init(sha_info);
    while ((e - p) > BLOCK_SIZE) {
		sha_update(sha_info, (unsigned char *) p, BLOCK_SIZE);
		p += BLOCK_SIZE;
    }
    if ((e - p) > 0)
		sha_update(sha_info, (unsigned char *) p, (e - p));	
    sha_final(sha_info);
}
